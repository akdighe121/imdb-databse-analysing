...
select year, count(title) no_of_movie
from movie
group by year;

select month(date_published) month, count(title)
from movie
...
select g.genre, m.year, count(*)  as no_movie
from movie m
...
sum(case when date_of_birth is null then 1 else 0 end) dob_nulls,
sum(case when known_for_movies is null then 1 else 0 end) known_nulls
from names;
...
with top_genre as (select genre,count(m.id) as movie_count,
                ...
                group by genre 
                limit 3)
select n.name director_name,count(d.movie_id) movie_count
from director_mapping d
join genre gn on d.movie_id=gn.movie_id
join names n on n.id=d.name_id
join top_genre using (genre)
...
rank() over(order by sum(total_votes) desc) prod_company_rank
from movie m
join ratings r 
on r.movie_id=m.id
group by production_company 
